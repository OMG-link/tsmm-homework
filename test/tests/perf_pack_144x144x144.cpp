...
static inline void _reorder_matrix_rhs(f64 *RESTRICT dst, const f64 *RESTRICT src, int k, int n, int src_line_stride) {
...
static inline void _pack_matrix_rhs(f64 *RESTRICT dst, const f64 *RESTRICT src, int k, int n, const int K_BLK,
                                    const int N_BLK) {
    for (int k_idx = 0, k_block; k_idx < k; k_idx += k_block) {
        k_block = min_int(K_BLK, k - k_idx);
        for (int n_idx = 0, n_block; n_idx < n; n_idx += n_block) {
            ...
            _reorder_matrix_rhs(dst_ptr, src_ptr, k_block, n_block, n);
        }
    }
}
...
void test(const Matrix &a, const Matrix &b) {
    ...
    pack_matrix_rhs(b_packed, b.data(), b.rows(), b.cols(), 144, 144);
