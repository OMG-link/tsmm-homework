...
static inline void _matmul_submat(f64 *RESTRICT dst, const f64 *RESTRICT lhs, const f64 *RESTRICT rhs, int m, int k,
    ...
    for (int m_idx = 0; m_idx < m; m_idx += OPK_M_BLK) {
        ...
        assert(n - n_idx == 8);
        {
            ...
            for (int i = 0; i < OPK_M_BLK; i++) {
                for (int j = 0; j < 8; j++) {
                    out[i * 8 + j] = dst[(m_idx + i) * dst_line_stride + (n_idx + j)];
                }
            }
            for (int k_idx = 0; k_idx < k; k_idx++) {
                for (int i = 0; i < OPK_M_BLK; i++) {
                    for (int j = 0; j < 8; j++) {
                        out[i * 8 + j] += lhs[(m_idx + i) * lhs_line_stride + k_idx] * rhs[n_idx * k + k_idx * 8 + j];
                    }
                }
            }
            for (int i = 0; i < OPK_M_BLK; i++) {
                for (int j = 0; j < 8; j++) {
                    dst[(m_idx + i) * dst_line_stride + (n_idx + j)] = out[i * 8 + j];
                }
            }
#endif
        }
